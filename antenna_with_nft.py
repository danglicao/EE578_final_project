...
def main():
    ...
    x_prob, y_prob, z_prob = 180e-3, 0, 0
    ...
    for n in tqdm(range(nt)):
        ...
        Dx, Dy, Dz, Ex, Ey, Ez, Hx, Hy, Hz, Bx, By, Bz, Dx_old, Dy_old, Dz_old, Bx_old, By_old, Bz_old = update_equations(
    ...
    # Z_f = V_f / I_f
    # print(Z_f)
    Z_f = np.zeros(nt, dtype = np.complex64)
    # plotting
    freqs_hz = np.fft.fftfreq(nt, d = dt.item())  # Hz
    freqs_ghz = freqs_hz * 1e-9
    # half = nt // 2
    # freqs_ghz = freqs_ghz[:half]
    # Z_f = Z_f[:half]
    omega = 2 * np.pi * freqs_ghz
    ...
    # plt.plot(freqs_ghz, Z_f.real, label = "Resistance", color = 'blue')
    # plt.plot(freqs_ghz, Z_f.imag, label = "Reactance", color = 'red')
    # plt.xlabel("Frequency (GHz)")
    # plt.ylabel("Input Impedance (Ohms)")
    # plt.title("Input Impedance of Dipole Antenna")
...
def update_equations(Dx, Dy, Dz, Ex, Ey, Ez, Hx, Hy, Hz, Bx, By, Bz,
    ...
    Dy[1:-1, 1:-1, 1:-1] = ((1 - dt / 2 * sigma_z_Dy) / (1 + dt / 2 * sigma_z_Dy)) * Dy[1:-1, 1:-1,
                                                                                     1:-1] + \
    ...
    Dz[1:-1, 1:-1, 1:-1] = ((1 - dt / 2 * sigma_x_Dz) / (1 + dt / 2 * sigma_x_Dz)) * Dz[1:-1, 1:-1,
                                                                                     1:-1] + \
    ...
    Bx[1:-1, 1:-1, 1:-1] = ((1 - dt / 2 * sigma_y_Bx) / (1 + dt / 2 * sigma_y_Bx)) * Bx[1:-1, 1:-1,
                                                                                     1:-1] + \
    ...
    By[1:-1, 1:-1, 1:-1] = ((1 - dt / 2 * sigma_z_By) / (1 + dt / 2 * sigma_z_By)) * By[1:-1, 1:-1,
                                                                                     1:-1] + \
    ...
    Bz[1:-1, 1:-1, 1:-1] = ((1 - dt / 2 * sigma_x_Bz) / (1 + dt / 2 * sigma_x_Bz)) * Bz[1:-1, 1:-1,
                                                                                     1:-1] + \
