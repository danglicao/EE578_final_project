...
def main():
    ...
    lambda_U = cp.float32(162.5) #mm
    lambda_L = cp.float32(87.5) #mm

    dx = dy = dz = cp.float32(4.0)  # mm, spatial step size
    ...
    frame_idx = 0
    Ex_frames = cp.zeros((n_frames, Nx+1, Ny), dtype = cp.float32)
    Ey_frames = cp.zeros((n_frames, Nx, Ny+1), dtype = cp.float32)
    ...
    for n in tqdm(range(nt)):
        ...
        if n % save_every == 0:
            Ex_frames[frame_idx] = Ex[:, :, kc]
            Ey_frames[frame_idx] = Ey[:, :, kc]
    ...
    # generate_field_animation(Ex_frames_cpu, save_every = save_every, save_path = "figs/animations/2.4ghzEx.mp4")
    # generate_field_animation(Ey_frames_cpu, save_every = save_every,
    #                          save_path = "figs/animations/2.4ghzEy.mp4")
    # generate_field_animation(Ez_frames_cpu, save_every = save_every,
    #                          save_path = "figs/animations/2.4ghzEz.mp4")
